    ...
    function step(dt) {
      ...
      if (g) {
        ...
        /* Die gezeichnete Höhe des Steins zieht der Regel weich nach. Zwischen zwei
           ...
           Nach OBEN wird nie nachgezogen, sondern sofort gesetzt: Höher rückt das
           Ziel allein dann, wenn der Anwender schiebt oder dreht und der Stein
           dabei auf etwas zu stehen kommt — der Vorlauf ins nächste Feld ist damit
           hinfällig. Weich hochzuziehen hieße, den Stein für ein paar Bilder in den
           Stapel gezeichnet zu lassen und danach sichtbar hochschnappen zu lassen.
           Sofort gesetzt fällt die Änderung mit dem Zug zusammen, den der Anwender
           ohnehin gerade sieht — und der Stein steht nie im Stapel. */
        if (g.piece && !g.over) {
          const target = g.piece.y + pieceFrac(g);
          if (vy === null || vyPiece !== g.piece || g.paused || target < vy || Math.abs(target - vy) > 2.5) {
            vy = target;
          } else {
